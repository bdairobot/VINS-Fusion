...
struct VIOFactorAuto
{
	...
	bool operator()(const T* const q_w_i, const T* p_i_xy, const T* p_i_z, const T* q_w_j, const T* p_j_xy, const T* p_j_z, T* residuals) const
	{
		...
		residuals[0] = (t_i_ij[0] - T(t_x)) / T(t_dev);
		residuals[1] = (t_i_ij[1] - T(t_y)) / T(t_dev);
		residuals[2] = (t_i_ij[2] - T(t_z)) / T(t_dev);
	...
	static ceres::CostFunction* Create(const double t_x, const double t_y, const double t_z,
									   ...
									   const double t_dev, const double q_dev) 
	{
	  return (new ceres::AutoDiffCostFunction<
	          VIOFactorAuto, 6, 4, 2, 1, 4, 2, 1>(
	          	new VIOFactorAuto(t_x, t_y, t_z, q_w, q_x, q_y, q_z, t_dev, q_dev)));
	}
